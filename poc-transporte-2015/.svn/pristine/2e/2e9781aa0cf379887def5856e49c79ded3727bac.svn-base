...
class CreateRetweetsRelationshipsGraph:
    ...
    def create_rt_graph(self, tweets):
        g = nx.DiGraph()
        for tweet in tweets:
            rt_origins = self.get_rt_origins(tweet)
            if not rt_origins:
                continue
    ...
    def write_d3js_output(self, g, out_file, word):
        nodes = g.nodes()
        indexed_nodes = {}
        idx = 0
        for n in nodes:
        ...
        finalgroups = {}
        while True:
            finalgroups = self.getMergeGroups(nodes, finalgroups, groups)
        ...
        f.close()
        #save redis
        self.save_to_redis(word, json_data)
        print >> sys.stderr, 'Data file written to: %s' % f.name

    def getMergeGroups(self, nodes, finalgroups, groups):
        idx = 0
        for n in nodes:
            glist = []
    ...
    def __init__(self, word, idSolr, ipRedis):
        try:
            global r_server
            r_server = redis.Redis(host=ipRedis)
            
            # Your Mongo query
            connection = pymongo.Connection(idSolr)
            db = connection.sptrans
            all_tweets = db['tweets'].find({"$text":{ "$search":word}})

            g = self.create_rt_graph(all_tweets)
            ...
            self.write_d3js_output(g, word+'.json', word)
            print "Success"
        except:
            print "Error"
